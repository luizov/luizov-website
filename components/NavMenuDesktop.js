...
import { IconDownSmall, IconOne, IconTwo } from '../components/base/Icons';
				...
				{({ open }) => (
					<>
						<Popover.Button
							className={cn(
								open
									? 'text-slate-11 dark:text-slateDark-11 bg-slate-3 dark:bg-slateDark-3'
						...
						>
							<Popover.Panel className="absolute z-10 right-0 mt-6 px-2 w-screen max-w-sm sm:px-0">
								<div className="rounded-lg shadow-lg border border-slate-6 dark:border-slateDark-6 overflow-hidden">
									<div className="relative grid gap-6 bg-white dark:bg-slateDark-2 px-5 py-6 sm:gap-8 sm:p-6">
										{resources.map((item) => (
											<Link
												key={item.name}
												href={item.href}
									...
									<div className="bg-white dark:bg-slateDark-1 border-t border-slate-6 dark:border-slateDark-6">
										<div className="flex justify-between items-center pt-3 pb-4 px-6 pr-4">
											<span className="text-slate-11 dark:text-slateDark-11">Dark Mode</span>
											<ThemeChanger />
										</div>
									</div>
