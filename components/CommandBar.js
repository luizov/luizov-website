    ...
    <KBarProvider actions={actions}>
      <KBarPortal>
        <KBarPositioner className="fixed inset-0 z-50 bg-white bg-opacity-80 dark:bg-black dark:bg-opacity-80">
          <KBarAnimator className="max-w-3xl w-full bg-slate-1 dark:bg-slateDark-2 rounded-md shadow-lg border border-solid border-slate-6 dark:border-slateDark-6 overflow-hidden">
            <KBarSearch
  ...
  return (
    <KBarResults
      ...
      onRender={({ item, handlers, active }) =>
        typeof item === "string" ? (
          <div className="border-l-2 border-transparent px-4 py-4 font-medium text-xs uppercase tracking-wider text-slate-11 dark:text-slateDark-11">
            {item}
          </div>
        ) : (
          <ResultItem
...
const ResultItem = React.forwardRef(({ action, handlers, active }, ref) => {
  return (
    <div
      ...
      className={cn(
        active
          ? 'border-blue-9 bg-slate-4 text-slate-12 dark:border-blueDark-9 dark:bg-slateDark-4 dark:text-slateDark-12'
          : '',
        'px-4 py-2 flex items-center justify-between border-l-2 border-transparent cursor-pointer text-slate-11 dark:text-slateDark-11'
      )}
      {...handlers}
    >
      {action.icon}
